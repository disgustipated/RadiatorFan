...
void pumpRunning()
{
  ...
  currMillis = millis();
  if ((digitalRead(FAN_ACTIVATE_PIN) == LOW) && (currMillis > (deviceActivateStart + ACTIVATE_DURATION))){
    stopPump();
    }
}
...
void activatePump()
{
  ...
  digitalWrite(FAN_ACTIVATE_PIN, LOW); //reversed these to prevent relay from going on during a reboot
